...
def make_fits(file, thetas, phis, mass = True, sb=True, temp=True, width=(5, 'Mpc'), suffix='', snapmin=-1, snapmax=1210, proj=True):
...
if __name__=="__main__":
    ...
    if files:
        for file in files:
            filenum = int(file.split('_')[1].split('.')[0])
            make_fits(file, thetas, phis, snapmin = snapmin, snapmax = snapmax, width=(14, 'Mpc')